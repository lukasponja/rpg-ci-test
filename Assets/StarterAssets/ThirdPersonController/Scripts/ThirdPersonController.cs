    ...
    public class ThirdPersonController : NetworkBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (isLocalPlayer)
            {
                ...
                FindObjectOfType<EditorModeManager>().gameObject.SetActive(true);

                vCams.playerCam = CameraManager.instance.vCam1;
                vCams.editorCam = CameraManager.instance.vCam2;
            }
        ...
        private void Update()
        {
            ...
            if (_input.enterEditMode)
            {
                _input.enterEditMode = false;

                if (_input.playerInput.currentActionMap.name == "BuilderActionMap")
                {
                    Debug.Log("exit edit mode");
                    _input.playerInput.SwitchCurrentActionMap("Player");
                    vCams.playerCam.Priority = 1;
                    vCams.editorCam.Priority = 0;
                    OnEditorMod?.Invoke(false);
                }
                else
                {
                    Debug.Log("enter edit mode");
                    _input.playerInput.SwitchCurrentActionMap("BuilderActionMap");
                    vCams.editorCam.Priority = 1;
                    vCams.playerCam.Priority = 0;
                    OnEditorMod?.Invoke(true);
                }
